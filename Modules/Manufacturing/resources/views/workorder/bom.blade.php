@php
    // Single source of truth for Bills of Materials. Loaded here so the
    // Work Orders → BoM tab is self-contained within the dashboard shell.
    $bomType = request()->get('bomType', 'prebuilt');
    if (!in_array($bomType, ['prebuilt', 'packaging'])) $bomType = 'prebuilt';

    $allInventoryItems = \Modules\Inventory\Models\Item::with('category')
        ->orderBy('name')
        ->get(['id', 'sku', 'name', 'category_id']);

    $isPackagingCategory = function ($item) {
        $cat = strtolower((string) optional($item->category)->name);
        return str_contains($cat, 'packag') || str_contains($cat, 'packing');
    };

    // Classify a BoM by its components' category rather than a stored "type":
    // a BoM built from packaging-category items is a Packaging BoM.
    $packagingItemIds = $allInventoryItems->filter($isPackagingCategory)->pluck('id')->flip();
    $bomIsPackaging   = fn ($bom) => $bom->items->contains(fn ($it) => $packagingItemIds->has($it->inventory_item_id));

    $boms = \Modules\Manufacturing\Models\ProductBom::with('items')->latest()->get()
        ->filter(fn ($bom) => $bomType === 'packaging' ? $bomIsPackaging($bom) : ! $bomIsPackaging($bom))
        ->values();

    // Pre-builts section only offers PC components; Packaging section only
    // offers packaging/packing categorized items.
    $inventoryItems = $allInventoryItems->filter(function ($i) use ($isPackagingCategory, $bomType) {
        return $bomType === 'packaging' ? $isPackagingCategory($i) : !$isPackagingCategory($i);
    })->values();

    // Emitted as plain PHP array so the JSON is written with {!! !!} — avoids
    // Blade's @json directive choking on the multi-line closure. Consumed by bom.js.
...
<div class="flex flex-col h-full text-nexora-deep-navy">

    <div class="flex items-center justify-between flex-shrink-0 mb-3">
        <h1 class="font-heading font-medium text-xl text-nexora-navy-mid">Bill of Materials</h1>
        <div class="flex items-center gap-1 bg-nexora-slate-200 border border-nexora-corporate/50 rounded-full p-1">
            @foreach($sectionTabs as $key => $label)
                <a href="?page=orders&sub=boms&bomType={{ $key }}"
    ...
    <div class="flex gap-3 flex-1 min-h-0">
        ...
        <section class="w-[32%] flex-shrink-0 bg-nexora-off-white border border-nexora-corporate rounded-xl
            ...
            <h2 class="font-heading font-medium text-lg text-nexora-navy-mid">
                New {{ $bomType === 'packaging' ? 'Packaging' : 'Prebuilt' }} BoM
            </h2>
            <p class="text-xs text-nexora-navy-mid mt-1 mb-4 leading-relaxed">
                @if($bomType === 'packaging')
                    Defines what packaging materials are consumed per shipment/order.
                @else
            ...
            <form method="post" action="{{ route('manufacturing.boms.store') }}" class="flex flex-col gap-3">
                ...
                <div>
                    <label class="text-[10px] font-semibold text-nexora-slate-500 uppercase tracking-wider">
                        {{ $bomType === 'packaging' ? 'Packaging Components' : 'Inventory Components' }}
                    </label>
        ...
        {{-- RIGHT: Current BoM list (current section only) --}}
        <section class="flex-1 bg-nexora-slate-200 border border-nexora-corporate/50 rounded-xl
                        p-5 overflow-y-auto [&::-webkit-scrollbar]:hidden">
            <div class="flex items-center justify-between mb-4">
                <h1 class="font-heading font-medium text-xl text-nexora-navy-mid">
                    Active {{ $sectionTabs[$bomType] }} BoMs
                </h1>
            ...
            <div class="flex flex-col gap-3">
                ...
                @empty
                    <p class="text-xs text-nexora-navy-mid">
                        No {{ strtolower($sectionTabs[$bomType]) }} BoMs yet. Create one from {{ $bomType === 'packaging' ? 'packaging' : 'inventory' }} components.
                    </p>
                @endforelse
            </div>
        </section>
